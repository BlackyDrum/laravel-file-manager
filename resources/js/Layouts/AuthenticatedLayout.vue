...
const menuItems = ref([{ label: "Dashboard", url: "dashboard" }]);
...
const uploadEvent = () => {
    ...
    if (files.value.length > maxFileUploadCount) {
        toast.add({
            severity: "info",
            summary: "Info",
            detail: `You can only upload ${maxFileUploadCount} files at once`,
            life: 6000,
        });
        return;
    }
    ...
    if (currentStorageSize.value + uploadFileSize > maxStorageSize) {
        toast.add({
            severity: "info",
            summary: "Info",
            detail: `Total file size will exceed your storage limit of ${formatBytes(maxStorageSize, 0)}`,
            life: 6000,
        });
        return;
    }
        ...
        for (const file of files.value) {
            if (file.name.length > maxFileNameSize) {
                toast.add({
                    severity: "info",
                    summary: "Info",
                    detail: `The filename cannot be greater than ${maxFileNameSize} characters`,
                    life: 6000,
                });
                return;
            }
            if (userFile.name === file.name) {
                toast.add({
                    severity: "info",
                    summary: "Info",
                    detail: `You already have a file with the name ${file.name}`,
                    life: 6000,
                });
                return;
            }
            if (file.size > maxFileSize.value) {
                toast.add({
                    severity: "info",
                    summary: "Info",
                    detail: `File Size exceeds the limit of ${Math.floor(maxFileSize.value / 1024 / 1024)}MB`,
                    life: 6000,
                });
                return;
            }
...
<template>
    <div class="card flex justify-content-center">
        <Toast
            ...
            group="headless"
            class="max-lg:w-[22rem]"
            @close="close()"
        >
            <template #container="{ message, closeCallback }">
                <section
                    class="grid grid-cols-[10%,90%] p-4 w-full bg-[#191919] shadow-2"
                    style="border-radius: 10px"
                >
                    <div>
                        <i
                            class="pi pi-cloud-upload text-[#3B81F4] text-2xl"
                        ></i>
                    </div>
                    <div class="text-white font-medium">
                        ...
                        <div class="flex gap-3 mb-3">
                            <Button
                                ...
                                "
                                text
                                class="text-white p-0 font-medium"
                                @click="closeCallback"
                            ></Button>
                        </div>
                    </div>
        ...
        <nav class="mr-4">
                    ...
                    <div class="my-6">
                        <ul>
                            <li
                                ...
                                class="w-full mb-4 p-2 rounded-lg font-medium cursor-pointer"
                        ...
                        <div class="text-sm">
                            Used {{ formatBytes(currentStorageSize, 0) }} /
                            {{ formatBytes(maxStorageSize, 0) }}
                            <div>
                                <ProgressBar
                                    :value="
                                        (currentStorageSize / maxStorageSize) *
                                        100
                                    "
                                    :showValue="false"
                                    :style="{ height: '4px' }"
                ...
                <div>
                    <ul>
                        <li
                            ...
                            class="w-full mb-2 mt-4 p-2 rounded-lg font-medium cursor-pointer"
            ...
            >
                ...
                <div class="text-sm p-2">
                    ...
                    <div>
                        <ProgressBar
                            :value="(currentStorageSize / maxStorageSize) * 100"
                            :showValue="false"
                            :style="{ height: '4px' }"
                        ></ProgressBar>
                    </div>
                ...
                <div class="hidden lg:flex">
                    <div class="grow flex" v-if="$page.url !== '/profile'">
                        <AutoComplete
                            ...
                            v-model="filterInput"
                            @input="emit('filterInput', filterInput)"
                ...
                >
                    <div class="flex gap-2">
                        ...
                        <Button
                            ...
                            severity="success"
                            :disabled="
                                !files || files.length === 0 || uploadProcessing
                            "
                        ></Button>
                        <Button
                            @click="clearCallback()"
                ...
                <div v-if="files.length > 0">
                    <div class="flex flex-wrap p-0 sm:p-5 gap-5">
                        ...
                        >
                            ...
                            <span
                                class="font-semibold self-center max-lg:break-all"
                                >{{ file.name }}</span
                            >
                            <div class="self-center">
            ...
            <template #empty>
                <div class="text-[#9CA3AF]">
                    <div class="flex">
                        <div class="mx-auto">
                            <i
                                class="pi pi-cloud-upload border-2 border-[#BFC3CB] rounded-full border-circle p-5 text-8xl text-400 border-400"
                            />
                        </div>
                    </div>
